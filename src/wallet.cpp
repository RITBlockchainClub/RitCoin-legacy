...
// rit: optional setting to unlock wallet for block minting only;
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    printf("WalletUpdateSpent found spent coin %scpay %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                if (pcoin->nTime > nSpendTime)
                    continue;  // rit: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                if (nFeeRet < TX_FEE && nChange > 0 && nChange < CENT)
                {
                    int64_t nMoveToFee = min(nChange, TX_FEE - nFeeRet);
                    nChange -= nMoveToFee;
                    nFeeRet += nMoveToFee;
                }

                // rit: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    vector<CTxOut>::iterator position = wtxNew.vout.begin(); // + 0
                ...
                else
                {
                    ...
                    scriptChange.SetDestination(vchPubKey.GetID());
                    vector<CTxOut>::iterator position = wtxNew.vout.begin(); // + 0
                    wtxNew.vout.insert(position, CTxOut(0, scriptChange));
                }
...
// rit: check 'spent' consistency between wallet and txindex
// rit: fix wallet spent state according to txindex
...
// rit: disable transaction (only for coinstake)
