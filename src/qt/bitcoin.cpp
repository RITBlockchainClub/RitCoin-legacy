...
bool ThreadSafeAskFee(int64_t nFeeRequired, const std::string& strCaption)
{
    if(!guiref)
        return false;
    if(nFeeRequired < TX_FEE || nFeeRequired <= nTransactionFee || fDaemon)
        return true;
    bool payFee = false;
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occured. rit can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
// TODO: implement qtipcserver.cpp for Mac and Windows
    ...
    {
        if (strlen(argv[i]) >= 7 && strncasecmp(argv[i], "rit:", 7) == 0)
        {
    ...
    app.setOrganizationName("rit");
    app.setOrganizationDomain("ritcoin.org");
    if(GetBoolArg("-testnet")) // Separate UI settings for testnet
        app.setApplicationName("rit-Qt-testnet");
    else
        app.setApplicationName("rit-Qt");
