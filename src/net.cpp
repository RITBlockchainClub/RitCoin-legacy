...
void CNode::PushGetBlocks(CBlockIndex* pindexBegin, uint256 hashEnd)
{
    ...
    PushMessage("getblocks", pindexBegin->GetBlockHash(), hashEnd);
}
...
{
    // Wait for IRC to get it first - disabled with rit
...
CNode* ConnectNode(CAddress addrConnect, int64_t nTimeout)
{
    ...
    /// debug print
    //printf("trying connection %s lastseen=%.1fhrs\n",addrConnect.ToString().c_str(),(double)(addrConnect.nTime - GetAdjustedTime())/3600.0);
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "rit " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (true /*!fTestNet*/)  // rit enables dns seeding with testnet too
    {
...
bool BindListenPort(string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to port %d on this computer.  rit is probably already running."), ntohs(sockaddr.sin_port));
        else
...
bool StopNode()
{
    ...
    if (vnThreadsRunning[THREAD_RPCLISTENER] > 0) printf("ThreadRPCServer still running\n");
    if (fHaveUPnP && vnThreadsRunning[THREAD_UPNP] > 0) printf("ThreadMapPort still running\n");
    if (vnThreadsRunning[THREAD_DNSSEED] > 0) printf("ThreadDNSAddressSeed still running\n");
    if (vnThreadsRunning[THREAD_ADDEDCONNECTIONS] > 0) printf("ThreadOpenAddedConnections still running\n");
    if (vnThreadsRunning[THREAD_DUMPADDRESS] > 0) printf("ThreadDumpAddresses still running\n");
        while (vnThreadsRunning[THREAD_MESSAGEHANDLER] > 0 || vnThreadsRunning[THREAD_RPCLISTENER] > 0)
    {
        Sleep(20);
    }
    Sleep(50);
